...
namespace _05.Rubik_s_Matrix
{
    class RubiksMatrix
    {
        static void Main()
        {
            var dimensions = Console.ReadLine().Split(new string[] {" "}, StringSplitOptions.RemoveEmptyEntries)
                .Select(int.Parse)
                .ToArray();
            ...
            {
                var commandsToken = Console.ReadLine().Split(new string[] {" "}, StringSplitOptions.RemoveEmptyEntries);
                    ...
                    else
                    {
                        for (int r = 0; r < matrix.Length; r++)
                        {
                            for (int c = 0; c < matrix[0].Length; c++)
                            {
                                ...
                                {
                                    var currentElemnt = matrix[rowIndex][colIndex];
                                    matrix[rowIndex][colIndex] = element;
                                    matrix[r][c] = currentElemnt;
